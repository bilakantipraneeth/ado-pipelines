#!/bin/bash
set -e
echo "--- Starting Product Export Step ---"
...
JSON_PAYLOAD=$(cat <<EOF
{
  "outputConfig": {
    "bigqueryDestination": {
      ...
      "tableIdPrefix": "$PRODUCT_TABLE_ID_PREFIX",
      "tableType": "view"
    }
  }
}
...
API_ENDPOINT="https://retail.googleapis.com/v2alpha/projects/$GCP_PROJECT_ID/locations/global/catalogs/default_catalog/branches/$PRODUCT_BRANCH_ID/products:export"

echo "Triggering product export to $API_ENDPOINT..."
...
if [ "$HTTP_STATUS" -eq 200 ]; then
  echo "Product export initiated successfully."
  ...
  echo "##vso[task.logissue type=error;]Failed to initiate product export. Received HTTP status code: $HTTP_STATUS. Error: $ERROR_MESSAGE.$SUGGESTION"
  exit 1
fi
